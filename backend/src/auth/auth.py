...
AUTH0_DOMAIN = 'dev-jg1rn0t0.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    ...
    if 'Authorization' not in request.headers:
        raise AuthError({'code': 'missing_authorization', 'description': 'Authorization does not exist in the header'}, 401)
    # get the token
    auth_header = request.headers['Authorization']
    header_parts = auth_header.split(' ')
    # check if token is valid
    if len(header_parts) != 2:
        raise AuthError({'code': 'invalid_authorization', 'description': 'Authorization header is not valid'}, 401)
    elif header_parts[0].lower() != 'bearer':
        raise AuthError({'code': 'invalid_authorization', 'description': 'Authorization header must start with "Bearer".'}, 401)
